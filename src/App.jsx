...
// Fondo Animado "Niebla/Humo" (Canvas)
const CanvasBackground = () => {
  ...
  useEffect(() => {
    ...
    // Creamos partículas más grandes y difusas para simular humo
    const createParticles = () => {
      ...
      for (let i = 0; i < particleCount; i++) {
        particles.push({
          ...
          radius: Math.random() * 200 + 100, // Radio muy grande para efecto humo
          speedX: (Math.random() - 0.5) * 0.3,
          speedY: (Math.random() - 0.5) * 0.3,
          alpha: Math.random() * 0.08 + 0.02, // Muy transparente
        });
      }
    };
    ...
    const drawParticles = () => {
      ...
      particles.forEach(p => {
        // Gradiente radial para suavizar bordes (efecto humo)
        const gradient = ctx.createRadialGradient(p.x, p.y, 0, p.x, p.y, p.radius);
        gradient.addColorStop(0, `rgba(200, 200, 200, ${p.alpha})`); // Centro grisaceo
        ...
        // Rebote suave en bordes extendidos para que no se corte la niebla
        if (p.x < -200 || p.x > canvas.width + 200) p.speedX *= -1;
        if (p.y < -200 || p.y > canvas.height + 200) p.speedY *= -1;
      });
  ...
  return (
    <canvas 
      ...
      style={{ filter: 'blur(40px)' }} // Blur extra CSS para suavizar aún más
    />
  );
};
...
export default function OpticaPro() {
  ...
  const handleCheckout = () => {
    const lineItems = cart.map(item => `• ${item.name} (${item.selectedColor}) x${item.quantity}: $${item.price * item.quantity}`).join('%0A');
    
    let deliveryInfo = "";
    if (orderData.deliveryType === 'delivery') {
      deliveryInfo = `🚚 *Envío a Domicilio*%0ADirección: ${orderData.street} ${orderData.number}, ${orderData.locality}`;
    } else {
      deliveryInfo = `🏪 *Retiro en Local*`;
    }

    const msg = `Hola OPTICA! 👋 Nuevo Pedido Web:%0A%0A${lineItems}%0A%0A💰 *Total: $${cartTotal}*%0A%0A👤 Cliente: ${orderData.name}%0A📞 Tel: ${orderData.phone}%0A${deliveryInfo}%0A📝 Notas: ${orderData.notes}`;
  ...
  return (
    <div className="relative min-h-screen font-sans text-zinc-900 bg-white selection:bg-black selection:text-white pb-0">
      {/* Fondo Animado Canvas - Efecto Humo */}
      <CanvasBackground />
      
      {/* Fondo Ruido Estático Sutil */}
      <div className="fixed inset-0 z-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-[0.03] pointer-events-none"></div>

      {/* NAVBAR */}
      <nav className="fixed top-0 left-0 right-0 z-40 transition-all duration-300 bg-black/95 backdrop-blur-sm border-b border-zinc-800">
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-24">
            <div className="flex items-center gap-3 cursor-pointer text-white group" onClick={() => scrollToSection('inicio')}>
              <div className="w-12 h-12 bg-white text-black flex items-center justify-center font-bold text-2xl tracking-tighter group-hover:bg-zinc-200 transition-colors">O</div>
              <span className="text-2xl font-bold tracking-[0.2em] uppercase">OPTICA</span>
            </div>

            {/* Desktop Menu */}
            <div className="hidden md:flex items-center space-x-12">
              <button onClick={() => scrollToSection('inicio')} className="text-xs font-bold uppercase tracking-widest text-zinc-400 hover:text-white transition-colors relative group">Inicio</button>
              <button onClick={() => scrollToSection('catalogo')} className="text-xs font-bold uppercase tracking-widest text-zinc-400 hover:text-white transition-colors relative group">Colección</button>
              <button onClick={() => scrollToSection('nosotros')} className="text-xs font-bold uppercase tracking-widest text-zinc-400 hover:text-white transition-colors relative group">Nosotros</button>
              <button onClick={() => scrollToSection('contacto')} className="text-xs font-bold uppercase tracking-widest text-zinc-400 hover:text-white transition-colors relative group">Contacto</button>
            </div>
        ...
        {/* Mobile Menu */}
        {mobileMenuOpen && (
          <div className="md:hidden bg-zinc-900 border-t border-zinc-800 p-4 absolute w-full shadow-2xl">
      ...
      {/* MAIN */}
      <main className="relative z-10 pt-40 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Hero Section */}
        <section id="inicio" className="mb-20 text-center lg:text-left lg:flex lg:items-center lg:justify-between min-h-[60vh] flex items-center">
          <div className="lg:w-1/2 mb-12 lg:mb-0 animate-fade-in-up">
            <span className="inline-block py-1 px-4 border border-black text-black text-[10px] font-bold uppercase tracking-[0.2em] mb-6">Colección 2025</span>
            <h1 className="text-6xl lg:text-8xl font-bold text-black mb-8 leading-none tracking-tight">
              VISION<br /><span className="text-zinc-400">REBELDE.</span>
            </h1>
            <p className="text-lg text-zinc-600 mb-10 max-w-md mx-auto lg:mx-0 font-medium leading-relaxed">
              Diseño industrial y materiales premium. No seguimos tendencias, las definimos.
            </p>
            <button 
              onClick={() => scrollToSection('catalogo')}
              className="bg-black text-white px-10 py-5 font-bold text-sm uppercase tracking-widest hover:bg-zinc-800 transition-all flex items-center justify-center gap-4 mx-auto lg:mx-0 group cursor-pointer"
            >
              Ver Catálogo <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
            </button>
          </div>
          <div className="hidden lg:block lg:w-1/2 pl-16 animate-fade-in-left">
             <div className="relative aspect-[4/5] overflow-hidden group bg-zinc-100">
               <img src="https://images.unsplash.com/photo-1511499767150-a48a237f0083?auto=format&fit=crop&q=80&w=1000" className="object-cover w-full h-full grayscale group-hover:grayscale-0 transition-all duration-700 ease-out" alt="Hero" />
        ...
        {/* Filtros */}
        <div id="catalogo" className="sticky top-24 z-30 bg-white/90 backdrop-blur py-6 mb-12 border-b border-zinc-100 flex gap-2 overflow-x-auto no-scrollbar scroll-mt-32">
          {["Todos", "Sol", "Receta", "Moda", "Deporte"].map(cat => (
            <button
              key={cat}
              onClick={() => { setActiveCategory(cat); setCurrentPage(1); }}
              className={`px-6 py-3 text-xs font-bold uppercase tracking-widest transition-all border border-transparent ${
                activeCategory === cat 
                ? 'bg-black text-white' 
        ...
        <div className="min-h-[600px]">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12 mb-16">
            {currentProducts.map(product => (
              ...
              >
                <div className="aspect-[5/4] bg-zinc-100 mb-6 relative overflow-hidden">
                  <img 
                    src={product.image} 
                    alt={product.name} 
                  ...
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/5 transition-colors flex items-center justify-center opacity-0 group-hover:opacity-100">
                    <span className="bg-white text-black px-6 py-3 text-xs font-bold uppercase tracking-widest transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                      Ver Detalle
                    </span>
                  </div>
                </div>
          ...
          {/* Paginación */}
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-4 py-12 border-t border-zinc-100">
              <button 
                onClick={() => paginate(currentPage - 1)} 
                disabled={currentPage === 1}
                className="p-3 border border-zinc-200 hover:border-black hover:bg-black hover:text-white disabled:opacity-20 disabled:border-zinc-200 disabled:hover:bg-white disabled:hover:text-zinc-300 transition-colors"
              >
                <ArrowLeft size={18} />
              </button>
        ...
        {/* Sección Nosotros */}
        <section id="nosotros" className="py-20 border-t border-zinc-100 text-center max-w-3xl mx-auto px-6">
          <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-400 mb-4 block">Sobre la Marca</span>
          <h2 className="text-4xl font-bold mb-6">ARTESANÍA MODERNA</h2>
      ...
      <footer id="contacto" className="bg-black text-white pt-20 pb-10 border-t border-zinc-800">
      ...
      <style>{`
        ...
        @keyframes pulse { 0%, 100% { opacity: 0.8; } 50% { opacity: 0.6; } }
